function requirecomponente(files, config, entrypath){
  ...
  function getContextComponent (namefile, ext, _path, objinfo){
      ...
      if(ext === ".jade" || ext === ".pug")
          resultcode = pug.render(resultcode,{
            filename:path.join(_path, namefile),
            pretty:  true
          });
  ...
  files.forEach(namefile => {
      ...
      _style = $('html').children('style');

      let typestylerender = $(_style).attr('lang');

      _style = $(_style).text();

      if(typestylerender === "sass")
          _style = _comp_sass.renderSync({data:_style}).css.toString('utf8');
          
      else if(typestylerender === "less"){
          _comp_less.render(_style, (err, output) => {
              if(err)
                  console.log(err);
              else {
                  _style = output.css;
      ...
      _script = _script == null ? '' : imports(_script, 2, config, _path, "nonregister");
      _scope = _scope == null ? '' : imports(_scope, 2, config, _path, "nonregister");
      ...
      _allStyle += _style !== undefined && _style !== null ? _style : '';
      // _allStyle = _allStyle.replace(/\s*|(\n\s+)/, "");
      ...
      if(type === "vue"){
          ...
          src_file = babel.transform(src_file, config.babel_compiler.options.react).code;
