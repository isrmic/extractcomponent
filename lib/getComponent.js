function requirecomponente(files, config){
  ...
  function getContextComponent (namefile, ext, objinfo){

      let pathfile = path.join(config.components_folder, namefile);
  ...
  files.forEach(namefile => {
      ...
      src_file = getContextComponent(namefile, ext, infos);

      if(src_file === null)
          return;

      if(infos.path !== '')
          _path = path.join(_path, infos.path);
      else
