function requirecomponente(files, config){
  ...
  files.forEach(namefile => {
      ...
      $ = cheerio.load('<html>' + src_file + '</html>', {decodeEntities: false, xmlMode: false, withDomLvl1:false});
      ...
      _script = _script == null ? '' : imports(_script, 2, config, path.join(process.cwd(), 'components', namefile));
      _scope = _scope == null ? '' : imports(_scope, 2, config, path.join(process.cwd(), 'components', namefile));
      ...
      if(type === "vue"){
          ...
          _component_vue.map((index, element) =>{
              ...
              //restart regex
              scopdata.exec("");              
              _components_vue[index] = `(function(){\n${_script_component}\nVue.component('${_name_component}', { template:`+'`'+`${_body_component}`+'`'+`, ${_scope_scomponent} });\n})();`;
