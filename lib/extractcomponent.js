var extract = function (__file_to_Extract, _config) {
  ...
  global.components = {allStyle:''};
  global.registermodules = [];
  global.registermodules["modules"] = [];
  global.registermodules["modulescomponent"] = [];
  global.registermodules["modulescomponentf"] = [];

  global._comp_sass = require('node-sass');
  global._comp_less = require('less');
  ...
  function run (){
      ...
      fs.readdir("./"+config.components_folder, (err, files) => {
          ...
          files_render.map(file =>{
              ...
              part_files = part_files.replace(/<{(.*?)}\/>/gi, function (x,y){
                  ...
                  if(AllComponents.vue[y] !== undefined){
                      ...
                      return Object.keys(config.babel_compiler.options.vue).length > 0 ?
                          babel.transform(`(function(){${AllComponents.vue[y + "scope1"] || ''}\nVue.component('${y}',${AllComponents.vue[y]})})();
          ...
          fs.writeFileSync(config.globalPath + "/" + config.build_stylesheet.save_folder + config.build_stylesheet.name, global.components.allStyle);

          final_file = imports(final_file, 2, config);

          final_file = final_file.replace("process.env.NODE_ENV", `'${config.NODE_ENV}'`);

          final_file = `(function(module, exports, _get_module){\nmodule.exports = exports;\n${global.registermodules["modulescomponentf"].join("")}\n${final_file}\n}).apply(this, [{}, {}, []])`;
    ...
    function returnex (file) {
        ...
        let endscopReturn = /exports[.]default/g.test(file) ? `\nreturn module.exports.default;` : /module[.]exports/g.test(file) ? '\nreturn module.exports;' : '\n';
