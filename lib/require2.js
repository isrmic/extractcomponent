    ...
    if(res !== null){
      res.map(matchs =>{
          ...
          if(nameload !== undefined && config.load_modules !== undefined && nameload in config.load_modules && condiction_nodemodules)
              ...
              script_in_module = fs.existsSync(path.join(_path, filename + ".js"));
          ...
          if((condiction_nodemodules || condiction_filemodule || script_in_module)){
             ...
             final_file = final_file.split(matchs).join(functionRequire);
             functionRequire = requiremodule(functionRequire, path.dirname(fileload), 1);
             if(patternrequire.test(final_file)){
                final_file = requiremodule(final_file, path.dirname(fileload), 1);
            }

          }
          else
              final_file = final_file.split(matchs).join("null");

      });
    }
    return patternrequire.test(final_file) ? requiremodule(final_file, rec_path, 1) : final_file;
}
else{
