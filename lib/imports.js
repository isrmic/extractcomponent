...
function importmodules (filetext, typerender, _config){
  ...
  patternImportsMatch.map(regex => {
      ...
      if(res !== null){
          ...
          res.map(codeforimport =>{
              typeimportvar = /{[^<>{}](.*?)}/gi.test(codeforimport) ? "blc" : "normal";
          ...
          importers.map(Import =>{
              ...
              if(fileload !== undefined && /{<(.*?)\/>}/i.test(fileload)){
                  ...
                  if(extension === ".html" || extension === ".pug" || extension === ".jade" || extension === ".vue" || extension === ".react"){
                      ...
                      if(fs.existsSync(path.join(_path, 'node_modules', fileload))){

                          let readpackagejson, _fileload;

                          requirefile = requiremodule(`require('${fileload}')`, process.cwd(), 2, fileload);
                          ...
                          requirefile = requiremodule(`require('${fileload}')`, process.cwd(), 2);
                          ...
                          (console.log(`modulo ${fileload} não encontrado !!! `), process.exit())
                  }
              }
              ...
              file_result = file_result.replace(Import.join(" "), (x, y) =>{
                  ...
                  if(typeload === "requirecomponent"){
                    ...
                    if(global.modules_register[fileload] !== undefined && included_pack[fileload] !== undefined){
                        call_module = global.modules_register[fileload]["call"];
                    }
                    else{
                        included_pack.push(fileload);
                        global.modules_register[fileload] = [];

                        global.modules_register[fileload]["call"] = `_getModule_${fileload.replace(/[^a-zA-Z ]|'|"|\//g, "_")}()`;
                        call_module = global.modules_register[fileload]["call"];
                        // functionrequiremodule = `${requirefile}\n \nvar ${main_var} = ${call_module};\n${prop_vars.join("")}\n`;
                        functionrequiremodule = `\nvar ${main_var} = ${requirefile}\n\n${prop_vars.join("")}\n`;
                        // call_module = global.modules_register[fileload]["call"];
                    ...
                    return `var ${main_var} = ${call_module};`;
                      ...
                      if(global.modules_register[fileload] !== undefined && included_pack[fileload] !== undefined){
                          ...
                          global.modules_register[fileload]["call"] = `_getModule_${fileload.replace(/[^a-zA-Z /]|'|"|\//g, "_")}()`;
                          call_module = global.modules_register[fileload]["call"];
                          functionrequiremodule = `${requirefile}\n\nnvar ${main_var} = ${call_module};\n${prop_vars.join("")}\nmodule.exports = ${main_var};`;
                          return functionrequiremodule;
                      }
  ...
  if(file_result !== undefined && file_result !== null){
      return requiremodule(file_result, process.cwd(), 1);
  }
  return "";
}
